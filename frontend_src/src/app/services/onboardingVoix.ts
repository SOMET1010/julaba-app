...
export const INTRO_CLIPS: Record<string, IntroClip> = {
  ...
  accueil: {
    ...
    atteste: false,
    prototype: true,
    // RÉENREGISTREMENT REQUIS (manifeste du pack prototype, lot A5) : l'audio
    // dit encore « Tata », le nom validé est « Tantie Nanti Lou ». `texte`
    // décrit ce que le fichier DIT et n'est donc PAS corrigé ici — le corriger
    // ferait mentir le registre sur son propre son. Texte cible :
    // « Akwaba. Pour vendre, touche un produit, ou parle à Tantie Nanti Lou. On est ensemble. »
