...
const PROFILES: Record<string, ProfileConfig> = {
  marchand: {
    ...
    steps: [
      ...
      { id: 'documents',   label: 'Documents', icon: FileText,    tip: 'NNI, CNI, CNPS, CMU' },
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        let data: any = [];
        try {
          data = await apiRequest<any>(API_URL, `/admin-divisions/districts`, { method: 'GET', signal: controller.signal });
        } catch (err) {
          if (err instanceof DOMException && err.name === 'AbortError') return;
          console.warn('[useAdminCascade] districts HTTP error:', (err as Error)?.message);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        let data: any = [];
        try {
          data = await apiRequest<any>(API_URL, `/admin-divisions/regions?district_id=${encodeURIComponent(districtId)}`, { method: 'GET', signal: controller.signal });
        } catch (err) {
          if (err instanceof DOMException && err.name === 'AbortError') return;
          console.warn('[useAdminCascade] regions HTTP error:', (err as Error)?.message);
          if (isMountedRef.current) setRegions([]);
          return;
        }
        if (!Array.isArray(data)) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        let data: any = [];
        try {
          data = await apiRequest<any>(API_URL, `/admin-divisions/departements?region_id=${encodeURIComponent(regionId)}`, { method: 'GET', signal: controller.signal });
        } catch (err) {
          if (err instanceof DOMException && err.name === 'AbortError') return;
          console.warn('[useAdminCascade] departements HTTP error:', (err as Error)?.message);
          if (isMountedRef.current) setDepartements([]);
          return;
        }
        if (!Array.isArray(data)) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        let data: any = [];
        try {
          data = await apiRequest<any>(API_URL, `/admin-divisions/communes?departement_id=${encodeURIComponent(departementId)}`, { method: 'GET', signal: controller.signal });
        } catch (err) {
          if (err instanceof DOMException && err.name === 'AbortError') return;
          console.warn('[useAdminCascade] communes HTTP error:', (err as Error)?.message);
          if (isMountedRef.current) setCommunes([]);
          return;
        }
        if (!Array.isArray(data)) {
...
/* ═══════════════════════════════════════════════════
   COMPOSANT PRINCIPAL
═══════════════════════════════════════════════════ */
const VALID_PROFILS: readonly ProfilType[] = ['marchand', 'producteur', 'cooperative'] as const;
...
export function FicheIdentificationDynamique() {
  ...
  const reverseGeocodeBackend = async (lat: number, lng: number) => {
    ...
    try {
      let result: any = null;
      try {
        result = await apiRequest<any>(API_URL, `/admin-divisions/reverse-geocode`, {
          method: 'POST',
          body: JSON.stringify({ lat, lng }),
          signal: controller.signal,
        });
      } catch (errGeo) {
        if (errGeo instanceof DOMException && errGeo.name === 'AbortError') return null;
        console.warn('[reverseGeocodeBackend] JSON parse failed:', errGeo instanceof Error ? errGeo.message : errGeo);
        return null;
      }
  ...
  const validateStep = (): boolean => {
    ...
    if (stepId === 'lieu') {
      if (!data.gps) {
        e.gps = 'Position GPS obligatoire. Clique sur "Détecter ma position".';
      }
    ...
    if (stepId === 'finalisation') {
      if (!data.gps) e.gps = 'Position GPS obligatoire. Clique sur "Capturer ma position GPS".';
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      if (isMountedRef.current) navigate('/identificateur/brouillons');
    } catch (e: unknown) {
      if (e instanceof DOMException && e.name === 'AbortError') return;
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Vérification PIN identificateur côté serveur
      if (data.codeIdentificateur) {
        try {
          ...
          try {
            ...
            if (errPin instanceof DOMException && errPin.name === 'AbortError') return;
            if (!isMountedRef.current) return;
          ...
          if (err instanceof DOMException && err.name === 'AbortError') return;
          console.warn('[FicheIdentificationDynamique] verify-pin failed:', err instanceof Error ? err.message : err);
      ...
      setField('numeroId', numId);
      submittedNumeroRef.current = numId;
      ...
      const safePhone = (data.telephone || '').startsWith('+225')
        ? (data.telephone || '')
        : '+225' + (data.telephone || '');
      ...
      if (isComplement) {
        try {
          ...
          if (acteurIdToUpdate) {
            try {
              await apiRequest(API_URL, `/users/${acteurIdToUpdate}`, {
                method: 'PATCH',
                body: JSON.stringify({
              ...
              if (errUser instanceof DOMException && errUser.name === 'AbortError') return;
              if (!isMountedRef.current) return;
          ...
          try {
            ...
            if (errIdent instanceof DOMException && errIdent.name === 'AbortError') return;
            if (!isMountedRef.current) return;
      ...
      // Branche création neuve - utilise endpoint transactionnel
      const acteurNom = `${data.prenoms} ${safeNom}`.trim();
      ...
      const payload = {
        acteur: {
          ...
          lastName: safeNom,
          role: profil === 'cooperative' ? 'cooperateur' : profil,
          commune: data.commune,
          market: data.marche,
          activity: data.produitsVendus || data.typeActivite,
          nin: data.nin || undefined,
          nationalite: data.nationalite || undefined,
          situationMatrimoniale: data.situationMatrimoniale || undefined,
          numCNPS: data.numCNPS?.trim() || undefined,
          numCMU: data.numCMU?.trim() || undefined,
          recepisse: data.recepisse || undefined,
          categorie: data.categorie || undefined,
          sousProfilMarchand: profil === 'marchand' ? (data.sousProfilMarchand || null) : null,
          typePointVente: data.typePointVente || null,
          typePointVenteAutre: data.typePointVente === 'autre' ? (data.typePointVenteAutre || null) : null,
          districtId: data.districtId || null,
          districtAutre: data.districtAutre || null,
          ...
          quartierVillage: data.quartierVillage || null,
          estMembreCooperative: data.estMembreCooperative === true,
          boitePostale: data.boitePostale || undefined,
          statutEntrepreneur: data.statutEntrepreneur || undefined,
          region: data.region || userRegion,
          zoneId: userZoneId,
          genre: data.genre || '',
          dateNaissance: data.dateNaissance || undefined,
          lieuNaissance: data.lieuNaissance || undefined,
          cooperativeName: data.nomCooperative || undefined,
        },
        identification: {
          type_acteur: profil === 'cooperative' ? 'cooperateur' : profil,
          statutEntrepreneur: data.statutEntrepreneur,
          market: data.marche,
          activity: data.produitsVendus || data.typeActivite || data.filierePrincipale || data.filiereCoopPrincipale,
          commune: data.commune || '',
          region: data.region || userRegion,
          latitude: data.gps?.lat ?? null,
          longitude: data.gps?.lng ?? null,
      ...
      try {
        let createData: any = null;
        try {
          createData = await apiRequest<any>(API_URL, `/identifications/create-with-acteur`, {
            method: 'POST',
            body: JSON.stringify(payload),
            signal: controller.signal,
          });
        } catch (errCreate) {
          if (errCreate instanceof DOMException && errCreate.name === 'AbortError') return;
          if (!isMountedRef.current) return;
        ...
        if (isMountedRef.current) toast.error('Erreur réseau lors de la création. Vérifie ta connexion et réessaie.');
        return;
      }
      ...
      if (data.photo && acteurId) {
        try {
          ...
          try {
            await apiRequest(API_URL, `/users/${acteurId}/photo`, {
              method: 'POST',
              body: photoFormData,
              signal: controller.signal,
            });
            ...
            if (errPhotoUp instanceof DOMException && errPhotoUp.name === 'AbortError') return;
            if (!isMountedRef.current) return;
            console.warn('[handleSubmit] Upload photo échoué:', errPhotoUp instanceof Error ? errPhotoUp.message : errPhotoUp);
            toast.warning('Acteur créé mais la photo n’a pas pu être téléversée. Tu peux la modifier plus tard.');
          }
        } catch (photoError: any) {
          if (photoError instanceof DOMException && photoError.name === 'AbortError') return;
          console.warn('[handleSubmit] Exception upload photo:', `acteurId=${acteurId}, message=${photoError?.message || ''}`);
          if (isMountedRef.current) toast.warning('Acteur créé mais la photo n’a pas pu être téléversée. Tu peux la modifier plus tard.');
        }
      }

      // PATCH email si présent (non bloquant : succès partiel toléré)
      if (data.email && acteurId) {
        try {
          await apiRequest(API_URL, `/users/${acteurId}`, {
            method: 'PATCH',
          ...
          if (emailError instanceof DOMException && emailError.name === 'AbortError') return;
          if (!isMountedRef.current) return;
          console.warn('[FicheIdentificationDynamique] PATCH email failed:', emailError instanceof Error ? emailError.message : emailError);
        }
      }
      ...
      <div className="min-h-screen pb-10 px-4 pt-6 lg:pl-[320px] max-w-2xl lg:max-w-3xl mx-auto" style={{ background: 'linear-gradient(180deg, #FAFAF8 0%, #FFFFFF 100%)' }}>
        ...
        <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-4 mb-8">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-gray-900" style={{ fontSize: '1.4rem', fontWeight: 900, lineHeight: 1.15, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Qui veux-tu identifier ?</h1>
            <p className="text-gray-500" style={{ fontSize: '0.88rem' }}>Sélectionne le profil pour démarrer.</p>
          </div>
        </motion.div>
        ...
        <div className="space-y-4">
          {(['marchand', 'producteur', 'cooperative'] as const).map((type, i) => {
            ...
            return (
              <motion.button
                key={type}
                onClick={() => { setProfil(type); setStep(0); }}
              ...
              >
                ...
                >
                  <Icon className="w-10 h-10" style={{ color: p.color }} aria-hidden="true" />
                </motion.div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-2 flex-wrap">
                    <div className="flex gap-0.5 flex-shrink-0">
                      {p.steps.map((_, si) => (
                        <div key={si} className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: p.color + '60' }} />
                      ))}
                    </div>
                    <span style={{ fontSize: '0.78rem', color: p.color, fontWeight: 600, whiteSpace: 'nowrap' }}>
                      {p.steps.length} étapes
                    </span>
                  </div>
                </div>
                <motion.div
                  ...
                  transition={prefersReducedMotion ? { duration: 0 } : { duration: 1.5, repeat: Infinity, delay: i * 0.3 }}
                >
                  <ArrowRight className="w-5 h-5" style={{ color: p.color }} aria-hidden="true" />
                </motion.div>
              </motion.button>
            );
          })}
        ...
        >
          ...
          <p className="text-blue-800" style={{ fontSize: '0.88rem' }}>
            Chaque dossier soumis sera vérifié avant création du compte.
          </p>
        </motion.div>
      </div>
    );
  ...
  return (
    <SubPageLayout role="identificateur" title="Fiche identification" noPadding onBackOverride={handleBack}>
...
function DocumentsStep({ data, setField, errors, cfg }: {
  ...
  const lookupNNI = async (nni: string) => {
    ...
    try {
      let result: any = null;
      try {
        result = await apiRequest<any>(API_URL, `/oneci/lookup/${nni}`, { method: 'GET', signal });
      } catch (err) {
  ...
  return (
    <div>
      ...
      <InputRow
        ...
        hint="Caisse Nationale de Prévoyance Sociale"
      />
      <InputRow
        label="Numéro CMU"
        fieldKey="numCMU"
...
async function syncZoneForCommuneLabel(
  ...
  try {
    let d: { zones?: Array<{ id: string; nom: string }> } | null = null;
    try {
      d = await apiRequest<{ zones?: Array<{ id: string; nom: string }> }>(API_URL, `/zones`, { method: 'GET', signal });
    } catch (err) {
      if ((err as Error)?.name === 'AbortError') return;
      return;
    }
...
function LieuStep({
  profil, data, setField, errors, color, handleGPS, gpsCapturing,
}: {
  profil: ProfilType;
  data: any;
  setField: (f: string, v: any) => void;
    ...
    <div className="space-y-5">
      ...
      {!data.gps ? (
        <motion.button
          ...
          whileTap={{ scale: 0.97 }}
          aria-label={gpsCapturing ? 'Capture en cours' : 'Détecter ma position'}
          style={{
            width: '100%',
            padding: '14px 16px',
        ...
        >
          ...
          {gpsCapturing ? 'Capture en cours…' : 'Détecter ma position'}
        </motion.button>
      ) : (
        <div style={{
          background: '#ECFDF5',
          ...
          {profil === 'cooperative' ? (
            ...
            <>
              ...
              >
                ...
                <RecapRow label="Catégorie" value={data.categorie} />
              </RecapBlock>
