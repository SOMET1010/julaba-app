...
//   → annulation admin → stock restauré → rejeu annulation (idempotence).
//
...
// navigue directement dans la caisse. La vente et l'annulation passent par les
// vrais endpoints ; le stock est lu via l'API et arbitré par la DB dans le
// script d'orchestration (run-recette.sh).
//
...
try {
  ...
  // Annulation (admin) → stock 100.
  const admin = await browser.newContext();
  await login(admin, ADMIN);
  const annul = await admin.request.patch(`${BASE}/api/v1/transactions/${txId}`, {
    data: { statut: 'annulee', motif: 'recette runtime — annulation espèces' },
  });
  const annulBody = await annul.json().catch(() => ({}));
  check('annulation admin acceptée (200)', annul.status() === 200, `HTTP ${annul.status()}`);
  check('restitution rapportée par l’API', Array.isArray(annulBody?.restitutions) && annulBody.restitutions.length === 1,
    JSON.stringify(annulBody?.restitutions || []));
  check('stock RESTAURÉ à 100 après annulation (API)', (await stockOf(ctx, PRODUIT)) === 100,
    'note : lecture API parfois transitoirement null — la DB fait foi (run-recette.sh)');
  await page.reload({ waitUntil: 'networkidle' }); await page.waitForTimeout(800);
  await page.screenshot({ path: `${OUT}/04-apres-annulation.png`, fullPage: true });

  // Idempotence : rejeu de l’annulation.
  const annul2 = await admin.request.patch(`${BASE}/api/v1/transactions/${txId}`, {
    data: { statut: 'annulee', motif: 'recette runtime — rejeu annulation' },
  });
  const annul2Body = await annul2.json().catch(() => ({}));
  check('rejeu annulation : aucune re-restitution', Array.isArray(annul2Body?.restitutions) && annul2Body.restitutions.length === 0,
    JSON.stringify(annul2Body?.restitutions || []));
