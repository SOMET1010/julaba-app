...
- Date de derniere mise a jour : 19/08/2026
...
19/08/2026, B2, voie negociation couverte par la reservation de stock (trou constate le 13/08/2026, corrige) :
- Cause racine confirmee par lecture de commandes-rest.controller.ts et negociation.entity.ts : l'entite Negociation ne stockait aucun lien vers la publication d'origine. Les commandes creees a l'acceptation d'une negociation (repondreNegociation, marchandRepondreContreOffre) etaient donc toujours creees avec publicationId = null, hors de portee de StockReservationService (conditionne a cmd.publicationId). Une marchande pouvait accepter plusieurs negociations sur le meme stock sans jamais etre bloquee par indisponibilite.
- Correctif : colonne negociations.publication_id (uuid, nullable, sans FK, meme pattern que commandes.recolte_id / commandes.negociation_id). Migration backend/src/database/migrations/1780900000000-NegociationPublicationLink.ts + miroir dans DbInitService.runInit() (meme raison que le miroir stock_reservations : synchronize/migrationsRun sont OFF sur la base Render peuplee). Verifie par npm run verify:dbinit-subsumed : DbInit n'ajoute rien au-dela de la chaine de migrations (724 objets, 0 ajoute/retire).
- Le front (MarcheVirtuel.tsx, seul point d'appel de proposerNegociation) envoie desormais publicationId = l'id de la publication negociee (deja disponible : c'est p.id de /publications/marche). Retro-compatible : sans publicationId, une negociation fonctionne comme avant (aucune reservation), donc les negociations "hors marche" ne sont pas cassees. Pas de backfill des negociations historiques (aucune reference fiable a reconstruire depuis les donnees existantes, cf. commentaire de la migration) : seules les negociations futures beneficient de la reservation.
- repondreNegociation et marchandRepondreContreOffre creent la commande CONFIRMEE avec ce publicationId, puis RESERVENT PUIS CONVERTISSENT le stock dans la MEME transaction que la mise a jour de la negociation (StockReservationService.reserver puis .convertir). Choix deliberement plus strict que le chemin "commande classique creee directement confirmee" (POST /commandes, qui ne fait que convertir, sans blocage) : ici reserver() bloque (409) si le disponible est insuffisant, ce qui est le point precis de ce correctif. Si le stock manque, toute la transaction est annulee : la negociation garde son statut precedent, aucune commande n'est creee.
- Consequence assumee : une commande nee de negociation est une vente ferme des l'acceptation (ligne stock_reservations 'convertie' d'emblee, recolte liee decrementee si applicable, publication passee a 'epuise' si le disponible tombe a 0). Annuler ensuite cette commande NE restitue PAS le stock, par la meme regle deja actee pour toute commande confirmee classique (section 10, "annulation d'une commande deja confirmee"). Verifie explicitement par le nouveau test (I-NEG-4) : le disponible restant continue de suivre normalement le cycle reservation/liberation d'une commande classique sur la meme publication.
- Bonus traite (mission "si le temps le permet") : le marchand peut desormais RE-CONTRE-OFFRIR (PATCH negociation/:id/marchand-repondre, statut 'contre_offre' en plus de accepte/refuse), dans la limite partagee de 3 contre-offres (nbContreOffres) deja prevue par le schema mais jamais atteignable en pratique jusqu'ici. Cote UI, seule InboxNegociations (vendeur) est deja compatible sans changement (elle affiche deja le prix courant via prixPropose et agit sur statut='en_attente') ; aucune modale marchand dediee n'a ete ajoutee pour declencher cette route (voir section 10).
- Tests : backend/test/invariants/negociation-reservation-stock.spec.ts (harnais test:invariants, Postgres), 5 cas : reservation+conversion a l'acceptation, blocage 409 d'un deuxieme acheteur sur le meme stock (negociation ET marchand-repondre), coherence du cycle classique en presence de stock deja consomme par une negociation, plafond des 3 contre-offres avec re-contre-offre marchand. Verifie vert en isolation ET dans la suite complete (14 suites / 80 tests, base Postgres dediee) ; aucune regression sur stock-reservation.spec.ts / argent-gele-b2.spec.ts / commande-negociation-lien.spec.ts.
- Aucun mouvement d'argent introduit ni touche par ce lot. No-Go Keiwa maintenu.
...
- B2, negociation : le marchand ne peut toujours pas re-contre-offrir plus d'une fois d'affilee sans que l'UI (InboxNegociations cote vendeur, MesCommandes cote marchand) propose un ecran dedie pour cette action. La route backend existe (PATCH negociation/:id/marchand-repondre, statut 'contre_offre', cf. decision du 19/08/2026) mais n'est pas encore branchee dans une modale marchand. A faire : UI marchand pour re-contre-offrir.
