...
14/08/2026, chantier socle : audit de derive de schema + convergence vers des migrations reproductibles.
- Constat : le schema de prod (Render) n'a aucun auteur unique. Il = ce que synchronize a cree quand la base etait vierge + les rustines idempotentes de DbInitService.runInit() (seul mecanisme qui fait evoluer le schema en prod, a chaque boot) + au moins un objet cree hors-bande. Les migrations ne tournent jamais (prepareDatabase force migrationsRun=false) et sont incompletes (elles ne creent pas ~20 tables et planteraient sur une base vide, CREATE TABLE non gardes). database/init.sql est mort et divergent (users_julaba, en_route, roles differents).
- Risque prod a verifier : POST /publications fait ON CONFLICT (user_id, LOWER(TRIM(produit))), mais l'index unique correspondant (ux_publications_user_produit) n'existe que dans un fichier de test. S'il n'a pas ete cree a la main sur la base Render, publier une offre echoue (500). A verifier au pg_dump.
- Plan de convergence (9 etapes, code-only puis prod-DB, chacune reversible) : (1) figer le schema reel par pg_dump --schema-only committe ; (2) neutraliser init.sql ; (3) une migration BaselineSchema unique reproduisant la prod ; (4) baseliner la table migrations (marquer l'existant applique) ; (5) reconcilier les enums ; (6) corriger cooperative_membres.cooperative_id varchar->uuid ; (7) dedupliquer cycles.status/statut ; (8) basculer migrationsRun en shadow ; (9) retirer synchronize-si-vide et DbInitService-comme-schema. Etapes prod-DB (4->9) uniquement avec feu vert explicite, backend d'abord, sauvegarde avant.
- Lot code-only livre (ce commit, zero impact sur la prod actuelle car synchronize off ; corrige la derive sur les builds neufs/tests) : enumName ajoute aux entites publications.statut (publication_statut), publications.type_marche (marche_virtuel_type_enum) et commandes.statut (commande_statut) ; cooperative_membres.cooperative_id et membre_id types en uuid (la FK se cree enfin sur base neuve/test) ; timestamp de migration duplique 1779200000000 renumerote (AddStockReservations -> 1779250000000) ; init.sql marque LEGACY.
