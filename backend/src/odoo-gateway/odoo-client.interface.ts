 ...
 * - `OdooMockClient` (ce lot) : simule le comportement, aucun réseau, aucun
 *   secret. Voir odoo-mock.client.ts.
 * - `OdooRealClient` (lot futur, PAS créé dans ce POC) : appellera
 *   réellement `POST {ODOO_BASE_URL}/json/2/<model>/<method>` avec :
 *     - `Authorization: bearer {ODOO_API_KEY}`
 *     - `Content-Type: application/json`
 *     - `X-Odoo-Database: {ODOO_DB}` (uniquement si plusieurs bases
 *       partagent le domaine)
 *   `ODOO_BASE_URL`/`ODOO_API_KEY`/`ODOO_DB` resteront des variables
 *   d'environnement BACKEND UNIQUEMENT — jamais exposées au frontend
 *   JULABA, jamais transmises au navigateur.
 *
