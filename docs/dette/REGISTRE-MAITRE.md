...
| **DEP-01** | **P1 actif** | **OUVERT** | *Ouverte à la révision 24, révélée par le contre-audit de la PR #245, vérifiée sur `e00a16c`.* **Toute dépense saisie depuis le frontend perd son libellé.** `frontend_src/src/app/contexts/CaisseContext.tsx` l. 486 envoie `{ montant, notes, idempotency_key }` ; `backend/src/caisse-rest/caisse-rest.controller.ts` l. 603 lit `body.description \|\| ''` ; l'entité `caisse-transaction.entity.ts` n'a que `description`. **Même perte au rejeu de la file hors ligne** (payload `notes` rejoué tel quel). Reproduit par lecture ; aucun test rouge n'existe | Présent depuis l'import initial, réécrit tel quel depuis. La PR #245 corrige ce point (`description ?? notes` côté backend, envoi de `description` côté frontend) **mais en zone interdite (backend)** : la correction plateforme est à faire séparément, avec un invariant sur `POST /caisse/depense` **et** sur le rejeu d'une file `notes` | **Condition de clôture :** libellé conservé sur les deux chemins (direct et rejeu), prouvé par un test rouge → vert. Dettes voisines : ARG-11 (`AppContext`), TEST-05 (tout invariant backend), API-07/08 |
...
| **UI-06** | P2 | **OUVERT** | *Ouverte à la révision 27 (lot A8), mesurée au banc sur `c527797`.* **Aucune carte produit ne tient entièrement dans le premier écran quand le panier n'est pas vide.** `apercu-caisse/capture.mjs`, 390 × 844, défilement 0, panier de démonstration (6 articles, 2 900 F) : la grille commence à **y = 708** et la première rangée de cartes occupe **708 → 905** — il manque **61 px**. Les quatre cartes rendues sont coupées : la photo et la pastille de stock sont vues, **le nom et le prix sont sous le pli** (capture `caisse-portrait-A8.png`). Le banc **rougit sur la condition 1/4**, « surface de vente compréhensible ». **Cause** : le raccourci panier du lot A9 (`.caisse-panier-raccourci`, 520 → 590 plus sa marge) s'insère **entre la zone voix et la grille** et repousse celle-ci de ~94 px ; panier vide, il n'est pas rendu et la première rangée tient | **Ce lot NE CORRIGE PAS ce défaut** : remonter la grille est une modification de mise en page de la caisse, hors du périmètre. Trouvé **parce que** le banc mesure désormais la règle fonctionnelle de Patrick et non plus une hauteur : l'ancienne règle ne regardait pas les cartes. **Vérifié dans les deux sens** : avec 70 px de moins au-dessus (triche à chaud, non commitée), les quatre conditions passent et le banc sort en 0 — le rouge est bien une mesure, pas un banc bloqué | **À trancher par Patrick**, comme UI-03 avant elle : (a) accepter qu'une carte soit coupée quand le panier n'est pas vide — et alors ajuster la condition 1/4 à ce que « compréhensible » veut dire pour lui ; ou (b) récupérer 61 px au-dessus (zone voix, bulle, rangée filtre + « Autre article », marge du raccourci) ; ou (c) raccourcir la carte de l'aperçu replié sous 1024 px. **Condition de clôture :** `node apercu-caisse/capture.mjs` sort en 0 sur la branche, sans triche. Mesures headless, aucun appareil réel |
