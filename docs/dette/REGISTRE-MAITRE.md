...
**Révision 35 — lot B6 : le hors ligne du service worker, repris et MESURÉ (OFF-04), sur `4eebfd6`.** L'intention de Manus était juste — polices et images disponibles hors ligne — son coût ne l'était pas : 4,6 Mo de pré-cache portés à 13–18 Mo à la première installation, sur données mobiles, sur des téléphones qui ont parfois 300 Mo libres. **Ce lot livre le chiffre, pas l'intention.** Mesuré sur le build : **10,19 Mo → 10,45 Mo**, **+271 Ko** pour les 10 polices (+2,6 %) ; les 4 gros chunks **restent dehors** (1,84 Mo) ; les **137 clips de voix (5,62 Mo) sont intacts** — promesse terrain, ils ne devaient ni être recomptés ni sortir par mégarde. `/images/*` passe **cache d'abord sans être pré-caché** : coût d'installation nul, et fin de l'attente sur un réseau mort mais non coupé. **Deux gates** au lieu d'une intention : `check:precache-budget` mesure après build et refuse au-delà de 12 Mo (dont 6 Mo bloquant), ou si la voix tombe sous 100 clips ; `test:precache-doctrine` tient la règle dans `verify` sans build. **Ce qui reste non prouvé et se mesure au terrain** : le temps d'installation réel sur réseau lent — on borne la taille, pas le débit. **Constat signalé, hors périmètre** : une quinzaine d'images référencées par le code sont absentes du build. **Chemin d'argent non touché.** `test:ci` **gelée**. **Aucune dette fermée par effet de bord** : OFF-02, VOIX-04, VOIX-05, UI-05, ARG-11, TEST-05 et GARDE-03 restent OUVERTES. Compte recompté par script : **112 lignes — 46 FERMÉ / 5 HORS PÉRIMÈTRE JUSTIFIÉ / 61 OUVERT** (avant : 111 — 45 / 5 / 61).

**Révision 34 — lot B5 : le niveau de voix ne peut pas taire l'argent (VOIX-08), sur `caaceae`.** Réglage repris de l'inventaire Manus (B5), **mécanisme entièrement réécrit sur notre socle**. Ce qui est refusé, mesuré sur notre catalogue : `importancePourTexte` devinait l'importance par une expression régulière **française** sur le texte dit — **42 des 120 messages `critiqueArgent` n'en contiennent aucun mot** et auraient donc été tus, et 140 clés sur 481 reçoivent du texte une importance différente de celle que le catalogue déclare ; en langue locale la règle ne reconnaît plus rien. S'y ajoutaient un niveau 0 coupant tout et « Essentiel » par défaut après connexion. **Sur notre socle** : l'importance est LUE AU CATALOGUE (dérivée de `critiqueArgent`, aucune importance inventée pour les 475 clés), **l'argent ne se tait à aucun niveau**, une clé non déclarée ne se tait jamais (le doute fait parler), **il n'existe aucun niveau qui coupe tout**, le défaut est **« complet »**, et tout silence part au journal (`TTS_IGNOREE`, raison `niveau-voix` — limite L4 de GARDE-02 traitée pour ce chemin). La décision est prise dans `creerSpeakMessage`, le dernier endroit où la CLÉ est encore connue : une fois le texte produit, il ne reste qu'à deviner. **Rouge d'abord, 2 échecs** sur l'entonnoir réel ; les trois règles sont prouvées sur le catalogue entier. **À DÉFINIR, dit et non inventé** : le catalogue ne distingue pas une alerte non financière d'un accompagnement pur ; « Moins bavard » tait donc les deux, ce qui n'est acceptable que parce que ce niveau n'est jamais imposé. **Observation remontée** : le mute global préexistant coupe tout, y compris une relecture de montant — non touché, à arbitrer. Référence de `test:voix-trace-source` régénérée par son mécanisme documenté, **vérifiée sémantiquement** : seule l'empreinte d'`AppContext.tsx` bouge, **un seul** appel de parole ajouté — `speak(v ? t('REGLAGE_VOIX_ESSENTIEL') : t('REGLAGE_VOIX_COMPLET'))`, une lecture de CLÉS et non un littéral —, **aucun retiré**, ordre des autres inchangé ; l'inventaire revient à **266 phrases distinctes**, exactement son compte d'avant le lot. `test:ci` **gelée** ; le nouveau test entre dans `verify` (`test:niveau-voix`) ; inventaire voix régénéré (417 → 418). **Aucune dette fermée par effet de bord** : GARDE-02, VOIX-04, VOIX-05, OFF-02, UI-05, ARG-11, TEST-05 et GARDE-03 restent OUVERTES. Compte recompté par script : **111 lignes — 45 FERMÉ / 5 HORS PÉRIMÈTRE JUSTIFIÉ / 61 OUVERT** (avant : 110 — 44 / 5 / 61).

**Révision 33 — lot B4 : une action de commande sans réseau se refuse clairement (OFF-03), sur `8f3da13`.** Défaut relevé par l'inventaire Manus (B4) et **reproduit sur notre socle** : les six gestes de `MesCommandes.tsx` faisaient `speak(e.message)` sur un échec, si bien que **la marchande entendait « Failed to fetch »** — sans savoir que rien n'était parti ni qu'il fallait recommencer. **Rouge d'abord, 18 échecs**, sur un test qui EXTRAIT chaque gestionnaire du vrai écran et l'EXÉCUTE avec des appels d'API espionnés. **Corrigé sur le socle Claude** : refus AVANT l'envoi quand le réseau est déjà connu absent, et rattrapage du cas traître (le téléphone se croit connecté, l'envoi tombe) — les deux moments, comme OFF-01 l'a appris. Classement par statut HTTP, jamais par le texte : un 4xx garde son message métier, c'est lui qui porte l'information utile. **Point dur vérifié avant tout le reste, et prouvé par exécution : aucune commande ne peut être perdue ni comptée deux fois** — hors ligne l'API n'est pas appelée, aucun état local ne bouge, et rien n'est mis en file (choix écrit : une action de commande se négocie à deux). `CommandeContext` n'a aucune écriture optimiste, vérifié dans le même test. **Chemin d'argent non touché** (`node ci/garde-argent.mjs` : périmètre inchangé, 86 fichiers, aucun fichier du noyau dans le diff). `test:ci` **gelée** ; le nouveau test entre dans `verify` (`test:refus-hors-ligne-commande`) ; inventaire voix régénéré (415 → 417 sites d'appel). **Aucune dette fermée par effet de bord** : OFF-02, VOIX-04, VOIX-05, UI-05, ARG-11, TEST-05 et GARDE-03 restent OUVERTES. Compte recompté par script (`docs/dette/compter-registre.mjs`) : **110 lignes — 44 FERMÉ / 5 HORS PÉRIMÈTRE JUSTIFIÉ / 61 OUVERT** (avant : 109 — 43 / 5 / 61).

**Révision 32 — lot B3 : le micro lent au démarrage sur Android, fermé (VOIX-07), sur `8e34c99`.** Défaut relevé par l'inventaire Manus (B3) et **reproduit sur notre socle** : la sonde du moteur vocal émise au démarrage par `main.tsx` tombe, sur Android, avant l'enregistrement du plugin natif ; son « non » était **mémorisé pour toute la session** (`probePromise` gardée telle quelle — seule une exception la remettait à zéro), si bien que le micro restait mort jusqu'au prochain lancement alors que le moteur était là une seconde plus tard. **Rouge d'abord, 5 échecs**, sur un test qui fait TOURNER le vrai module `offlineStt.ts` — rechargé par génération, seul `nativeStt.isAvailable` étant piloté — et qui compte les sondes réellement émises. **Corrigé sur le socle Claude** : `verifierOfflineModel()` de Manus lu et **non repris** (le défaut est dans la mémoïsation, pas dans l'absence d'une fonction ; on ne crée pas de surface publique de plus). Ne sont désormais mémorisés qu'un OUI et la sonde en vol. **Les trois garanties du cache restent prouvées dans le même test** : pas de resonde après un OUI, une seule sonde pour cinq appels simultanés, rattrapage après une sonde qui lève ; et le drapeau persistant n'est écrit que sur un succès. **Chemin d'argent non touché** (`node ci/garde-argent.mjs` : périmètre inchangé, 86 fichiers, aucun fichier du noyau dans le diff) — pas de contre-audit requis par la méthode. `test:ci` **gelée** ; le nouveau test entre dans `verify` (`test:sonde-moteur-voix`). La référence de `test:voix-trace-source` a été régénérée par son propre mécanisme documenté — seule l'empreinte de `offlineStt.ts` bouge, l'inventaire des appels de parole est identique, aucune assertion desserrée. **Aucune dette fermée par effet de bord** : VOIX-04, VOIX-05, OFF-02, UI-05, ARG-11, TEST-05 et GARDE-03 restent OUVERTES. Compte recompté par script (`docs/dette/compter-registre.mjs`) : **109 lignes — 43 FERMÉ / 5 HORS PÉRIMÈTRE JUSTIFIÉ / 61 OUVERT** (avant : 108 — 42 / 5 / 61).

**Révision 31 — lot B2, arbitrage haptique tranché par Patrick : VOIX-06 FERMÉE sur `9d0da86`.** « Oui, mets une vibration courte unique pour l'état gardée hors ligne / en attente. Une seule impulsion, distincte du succès et de l'erreur. » `vibrerAttente()` = **90 ms, impulsion unique** : moitié de l'erreur (180 ms), un seul coup là où le succès en fait deux de 35 ms — la confusion est écartée par la FORME du motif, pas par un pari sur l'apprentissage. Appelée dans la branche `en_attente` de `POSCaisse.handlePay`, où `vibrerSucces()` reste inatteignable. **Rouge d'abord** (« haptique si en attente : rien », 2 échecs), puis vert : la garde `test:vente-hors-ligne-statut` lit l'écran réel par son arbre syntaxique, écarte le `catch` — troisième issue, qui ne dit rien du statut — et **exécute le vrai module haptique** pour comparer les trois motifs. Les trois canaux (écran, voix, toucher) portent de nouveau l'état de la vente, cette fois sans mentir. **Dette résiduelle nommée, non fermée** : l'écran de fin n'est toujours pas une région annoncée (`role="status"`) — autre population, autre arbitrage, laissé à Patrick. **Aucune autre dette touchée** : OFF-02, UI-05, VOIX-04, ARG-11, TEST-05 et GARDE-03 restent OUVERTES. Compte recompté par script (`docs/dette/compter-registre.mjs`) : **108 lignes — 42 FERMÉ / 5 HORS PÉRIMÈTRE JUSTIFIÉ / 61 OUVERT** (avant : 108 — 41 / 5 / 62).

**Révision 30 — lot B2 : fermeture d'OFF-01, la vente hors ligne annoncée « réussie », sur `4943ee8`.** Une vente seulement mise en file était annoncée confirmée. `enregistrerVente` avait **trois issues** — téléphone hors ligne, serveur qui accuse réception, envoi tombé alors que `navigator.onLine` disait « en ligne » (le cas le plus traître) — qui rendaient toutes `undefined`, et `POSCaisse` **ne lisait pas le résultat** : écran vert « Vente réussie », `vibrerSucces()`, `TATA_VENTE_ENREGISTREE`, dans les trois cas. **Reproduit avant correction, 13 rouges sur l'invariant lui-même** (jamais sur un module absent ni sur un seed) : la garde `frontend_src/src/app/voice-offline/venteHorsLigneStatut.test.mts` **extrait le corps réel** d'`enregistrerVente` de `CaisseContext.tsx` (avec `genererCle` et `doitEnfiler`), le détype par le compilateur TypeScript et l'**exécute** sur la VRAIE file hors ligne (`enfilerOperation` + `synchroniser`, store mémoire), puis lit le VRAI `handlePay` par son **arbre syntaxique** ; les deux lectures sont confrontées — les statuts que l'écran teste doivent être ceux que la fonction exécutée vient de rendre. Rouges obtenus : les trois issues rendent `undefined` et sont indiscernables ; `Promise<void>` au contrat ; le résultat est jeté par `handlePay` ; `vibrerSucces()` reste atteignable sur une vente en attente ; aucune annonce propre à l'attente ; l'écran dit « réussie » sans condition. **Corrigé sur le socle Claude**, le module Manus `statutOperationCaisse` **lu comme référence et jamais importé** : `ResultatEnregistrement { statut: 'confirmee' | 'en_attente' }` est déclaré et produit dans `CaisseContext`, **là et nulle part ailleurs** ; les deux chemins d'attente rendent le **même** statut (ce qui compte n'est pas pourquoi la vente attend, c'est qu'elle attend) ; l'erreur métier 4xx continue d'être **levée**, ce n'est pas un statut. `POSCaisse` branche : **confirmée** = comportement inchangé (écran vert, `vibrerSucces()`, `TATA_VENTE_ENREGISTREE` / `_RUPTURE`) ; **en attente** = écran « Vente gardée sur le téléphone / En attente d'envoi », voix `TATA_VENTE_GARDEE_TELEPHONE` et `TATA_VENTE_GARDEE_TELEPHONE_RUPTURE` (clés de catalogue `fr-ci`, `critiqueArgent`), **aucune vibration** et aucun message de succès définitif. **Le panier est vidé dans les deux cas** — la vente est prise en charge, jamais à ressaisir ; ni `enfilerOperation`, ni `genererCle()`, ni l'idempotence, ni le décrément de stock serveur ne sont touchés. `test:ci` **gelée**, identique valeur contre valeur à `f0c965c` ; le nouveau test entre dans `verify` (`test:vente-hors-ligne-statut`), pas dans `test:ci`. **LE REÇU, décision de Patrick intégrée au lot :** le bouton de partage reste disponible hors ligne — la vente a eu lieu devant la cliente et elle a droit à sa trace — mais `texteRecu` porte alors **une ligne de plus**, « Vente enregistrée sur ce téléphone — synchronisation en attente. », et **jamais** un état définitif côté serveur. Une vente confirmée rend le reçu **identique à l'octet** à celui d'avant B2, prouvé en exécutant la version de `recu.utils.ts` **relue dans git à `4943ee8`** ; le reçu en attente en est le reçu confirmé **plus exactement une ligne insérée**, comparaison ligne à ligne — montant, monnaie, lignes, unités, numéro, date, mode de paiement et identité sont donc verrouillés par construction. Champ `statutSynchronisation`, **nom distinct** du `statut` comptable d'une vente (`validee`/`annulee`) : deux sens, deux noms. Le type `StatutEnregistrement` **déménage** dans `types/statutEnregistrement.ts`, module sans dépendance, et le contexte de caisse le **ré-exporte** — une seule déclaration, et un util pur qui ne tire pas React derrière lui. Rouge d'abord, 11 échecs. **« Mes ventes » n'est pas concerné, vérifié dans le code** : son historique vient de `getSalesHistory`, donc de transactions rechargées depuis le serveur, et le seul écrivain local (`AppContext.addTransaction`, `synced: false`) **n'a aucun appelant**. **TROIS DETTES OUVERTES, pas seulement nommées** — documenter une dette ne la ferme pas, et ne pas l'ouvrir ne la crée pas non plus : **OFF-02** (après synchronisation réussie, rien ne dit à la marchande que sa vente est partie ; `synchroniser` ne rend qu'un compte global, annoncer une vente sur le rejeu d'une dépense serait un nouveau mensonge) ; **VOIX-06** (l'état « gardée » n'a aucun canal non visuel garanti — B2 retire la vibration en même temps que le mensonge, et sur la base le succès mensonger vibrait au moins ; le motif haptique est un arbitrage de Patrick, posé, non tranché) ; **GARDE-03** (*constat du contre-audit, pas de l'auteur du lot*) : `ci/garde-argent.mjs` l. 762 vérifie que le fichier `--preuve` CONTIENT les chaînes de commande, jamais qu'elles ont tourné ni qu'elles sont vertes, et ce fichier n'est pas commité — le gate qui impose « la preuve doit traverser » repose, lui, sur la bonne foi. Non corrigé dans ce lot, volontairement : toucher le garde dans le même diff que l'argent est ce qu'il refuse. **Aucune dette fermée par effet de bord** : UI-05, VOIX-04, ARG-11 et TEST-05 restent OUVERTES. Compte recompté par script (`docs/dette/compter-registre.mjs`) : **108 lignes — 41 FERMÉ / 5 HORS PÉRIMÈTRE JUSTIFIÉ / 62 OUVERT** (avant : 105 — 40 / 5 / 60).
...
**DEP-01** et **OFF-01**, P1 actif, ouvertes ci-dessous à la date de cette révision — *toutes deux FERMÉES depuis (DEP-01 révision 29, OFF-01 révision 30)*. **Aucune ligne Manus
...
| ID | Gravité | Statut | Preuve actuelle (code) | Commit / justification | Dette résiduelle |
|---|---|---|---|---|---|
| **OFF-01** | **P1 actif** | **FERMÉ** | **Une vente qui n'est que gardée sur le téléphone ne s'annonce plus comme confirmée par le serveur.** `CaisseContext.enregistrerVente` rend désormais `ResultatEnregistrement { statut: 'confirmee' | 'en_attente' }` sur ses **trois** issues : hors ligne → `en_attente` ; accusé de réception serveur → `confirmee` ; envoi tombé alors que `navigator.onLine` disait « en ligne » → `en_attente`. L'erreur métier 4xx reste **levée**. `POSCaisse.handlePay` lit ce résultat et sépare écran, haptique et voix. Garde `frontend_src/src/app/voice-offline/venteHorsLigneStatut.test.mts`, branchée dans `verify` par `test:vente-hors-ligne-statut` — elle **extrait et exécute** le corps réel d'`enregistrerVente` sur la VRAIE file (`enfilerOperation` + `synchroniser`) et lit le VRAI `handlePay` par son arbre syntaxique, puis **confronte les deux lectures** : les statuts que l'écran compare doivent être ceux que la fonction vient de rendre ; rien n'est recopié à la main. **Rouges obtenus avant correction : 13** — les trois issues rendent `undefined` (indiscernables), `Promise<void>` au contrat, résultat jeté par `handlePay`, `vibrerSucces()` atteignable en attente, aucune clé de voix propre à l'attente, « réussie » affiché sans condition | **Lot B2, ce commit** — correction faite **sur le socle Claude**, pas par reprise du patch Manus : `statutOperationCaisse` (PR #245) a été lu comme référence et **n'est pas importé**. Une seule vérité : le statut est **produit** par `CaisseContext` et par lui seul — un écran ne redéduit jamais l'état d'une vente de `navigator.onLine`, qui ment quand l'envoi tombe. Voix : deux clés de catalogue `critiqueArgent`, `TATA_VENTE_GARDEE_TELEPHONE` et `..._RUPTURE` (« gardée sur le téléphone… pas encore envoyée… dès que le réseau revient »), jamais le mot « enregistrée ». Haptique : **aucune vibration** en attente — `vibrerSucces()` n'est pas réemployé sous un autre nom. Écran : icône, couleur et titre conditionnés au statut. Le panier est vidé dans les deux cas ; `enfilerOperation`, `genererCle()`, l'idempotence et le décrément serveur sont **intacts** | **Dette résiduelle :** la boucle « la file s'est vidée → alors seulement on confirme » **n'est pas fermée**. Après une synchronisation réussie, `CaisseContext` recharge les transactions mais **ne dit rien** à la marchande : sa vente gardée part en silence. La fermer proprement suppose de savoir **combien de VENTES** (et non d'opérations, dépenses et stock confondus) viennent de passer — `synchroniser` ne rend qu'un `{ ok, echecs, reste }` global — donc de toucher le module de synchronisation : hors du périmètre minimal de B2, à ouvrir comme ligne propre. **Arbitrages ouverts pour Patrick :** motif haptique distinct pour l'attente ; `role="status"` sur l'écran de fin. Dettes voisines, **non fermées** : UI-05 (raccourci panier non collant, 4 px de marge au premier écran), VOIX-04, ARG-11, TEST-05, et DEP-01 (fermée au lot B1, même `CaisseContext`, même file) |
| **OFF-02** | **P1 actif** | **OUVERT** | *Ouverte à la révision 30 (lot B2), reproduite par lecture du code sur `4943ee8`.* **Rien ne dit à la marchande, APRÈS synchronisation réussie, que sa vente gardée est enfin partie.** OFF-01 a fermé le mensonge à l'encaissement : une vente mise en file s'annonce désormais « gardée sur le téléphone ». Mais la deuxième moitié de la phrase de Patrick — « après synchronisation serveur réussie, seulement alors succès confirmé » — n'est pas tenue : dans `CaisseContext.tsx`, l'effet de synchronisation fait `if (ok > 0) await loadTransactions()` et **ne dit rien**. La vente part en silence ; l'écran de fin, lui, a déjà été refermé. Aucun test rouge n'existe, la dette est constatée par lecture | **Pourquoi ce n'est pas un oubli de B2, et pourquoi ce n'est pas trivial.** `offlineCaisse.synchroniser` ne rend qu'un `{ ok, echecs, reste }` **global**, toutes natures confondues : ventes, dépenses et mouvements de stock passent par la même file. Annoncer « ta vente est partie » sur un `ok > 0` qui était en fait une dépense serait **un nouveau mensonge du même genre que celui que B2 vient de fermer** — et sur le chemin d'argent, un mensonge de confirmation coûte plus cher qu'un silence. Le faire honnêtement suppose de compter par point de terminaison, donc de modifier le module de synchronisation hors ligne (zone `offline-synchronisation`, celle où naissent les doubles comptes) : c'est un lot à soi, pas un ajout au périmètre minimal de B2 | **Condition de clôture :** `synchroniser` rend de quoi distinguer ce qui vient de passer (au minimum un compte par point de terminaison), et la marchande l'APPREND — à la voix d'abord (clé de catalogue, `critiqueArgent`), pas seulement par un compteur qui descend — sans jamais annoncer une vente sur le rejeu d'une dépense ; prouvé par un test rouge → vert dans `verify` qui rejoue la VRAIE file avec un mélange de natures. Dettes voisines : OFF-01 (fermée, même `CaisseContext`, même file), VOIX-06, DEP-01, ARG-11 |
...
| **VOIX-06** | **P1 fonctionnel voix** | **FERMÉ** | **L'état « vente gardée sur le téléphone » se SENT, et son motif ne peut se confondre ni avec le succès ni avec l'erreur.** `utils/haptique.ts` : `vibrerAttente()` = **90 ms, impulsion unique** — la moitié de l'erreur (180 ms, unique), donc jamais lue comme une alarme ; un seul coup continu bien plus long que les 35 ms du succès (`[35, 60, 35]`, double), donc jamais pris pour une moitié de double impulsion. `POSCaisse.handlePay` l'appelle dans la branche `en_attente`, et `vibrerSucces()` y reste **inatteignable**. Prouvé dans `verify` par `test:vente-hors-ligne-statut` (T14), qui lit l'écran RÉEL par son arbre syntaxique — en écartant le `catch`, troisième issue qui ne dit rien du statut — puis **exécute le vrai module haptique** pour comparer les trois motifs : unicité, durée inférieure à l'erreur, différence avec le succès. **Rouge avant correction** : « haptique si en attente : rien », 2 échecs | **Lot B2, arbitrage de Patrick du 21/09/2026**, demandé par cette ligne et tranché par lui : « une vibration courte unique pour l'état gardée hors ligne / en attente. Une seule impulsion, distincte du succès et de l'erreur. » La prudence qui avait fait choisir l'absence de vibration — un motif inédit se confond avec le succès tant qu'il n'a pas été appris — est levée par la forme du motif elle-même, pas par un pari : une impulsion unique ne peut pas être une double. `vibrerSucces` n'est **pas** réemployé sous un autre nom. **Ce que ça répare vraiment** : B2 avait retiré le canal tactile en même temps que le mensonge, et l'état vrai naissait plus pauvre que l'état faux qu'il remplaçait ; les trois canaux (écran, voix, toucher) portent de nouveau l'information, cette fois sans mentir | **Dette résiduelle, nommée et NON fermée ici :** l'écran de fin n'est toujours pas une **région annoncée** (`role="status"` / `aria-live`) — arbitrage de doctrine voix laissé à Patrick, sciemment non tranché. Ce n'est pas l'objet de cette ligne : VOIX-06 mesurait l'absence de canal non visuel pour une marchande qui ne lit pas et peut ne pas entendre, et ce canal existe. L'annonce écran sert une autre population — une utilisatrice de lecteur d'écran ayant coupé Tata — et ferait dire deux fois la même chose tant que Tata parle. À ouvrir comme ligne propre si Patrick veut la traiter. Dettes voisines : OFF-01 (fermée), OFF-02, VOIX-04, VOIX-05 |
| **VOIX-07** | **P1 fonctionnel voix** | **FERMÉ** | *Ouverte ET fermée à la révision 32 (lot B3), défaut relevé par l'inventaire Manus (B3) et **reproduit sur notre socle** avant correction.* **Un « non » de la sonde de démarrage condamnait la voix pour toute la session — le « micro lent au démarrage » du terrain.** Sur Android, le plugin natif (sherpa-onnx) s'enregistre APRÈS le premier écran ; `main.tsx` sonde le moteur en tâche de fond, donc trop tôt, et `offlineStt.probeEngine()` gardait la promesse résolue à `false` **pour toute la session** : seule une exception remettait `probePromise` à zéro, jamais un `false`. `ensureOfflineModel()` relisait ce `false` et levait « La voix marche dans l'application Julaba installée sur le téléphone. » à chaque tentative, alors que le moteur était monté une seconde plus tard. Le chemin de rattrapage existait pourtant déjà (`useVoiceCore` l. 822-826 : si `offlineModelReady()` est faux, on rappelle `ensureOfflineModel()`) — il ne pouvait simplement jamais aboutir. **Rouge avant correction : 5 échecs** sur `voice-offline/sondeMoteurVoix.test.mts` (« après montage du plugin : sondes=1, ensureOfflineModel REFUSE ») | **Lot B3, ce commit** — correction **sur le socle Claude**. L'approche Manus (`verifierOfflineModel()`, une fonction exportée de plus) a été lue et **non reprise** : le défaut est dans la mémoïsation elle-même, on le corrige là, sans nouvelle surface publique — tous les appelants existants en profitent sans changer. `probeEngine()` ne mémorise plus qu'un OUI (`engineReady`) et la sonde EN VOL ; un `false` efface `probePromise`, donc la prochaine demande resonde. **Ce que la correction ne casse pas, et qui est prouvé dans le même test** : un moteur confirmé n'est plus resondé (1 sonde pour 4 appels), cinq appels simultanés au démarrage ne sondent qu'une fois, une sonde qui LÈVE se rattrape déjà (non-régression), et le drapeau persistant `julaba_offline_installed` n'est écrit que sur un succès — un « non » de démarrage ne déclare jamais l'appareil équipé. Le test fait TOURNER le vrai module, rechargé par génération, en ne pilotant que `nativeStt.isAvailable` : il compte les sondes réellement émises, il ne relit pas le source | **Une référence de garde-fou régénérée, et dite** : `test:voix-trace-source` gèle `offlineStt.ts` contre `3917bb7` pour prouver que l'instrumentation VOICE-01 n'avait rien changé à la voix ; une correction FONCTIONNELLE du même fichier le fait donc rougir. Régénérée par son **propre mécanisme documenté** (`node scripts/test-voix-trace-source.mjs --regenerer`, « après une évolution VOULUE et relue »), **aucune assertion retirée ni desserrée**, et vérifié ensuite que **seule** l'empreinte de `offlineStt.ts` a bougé et que **l'inventaire ordonné des appels de parole est identique** — la promesse de fond (aucune phrase ajoutée, retirée ou déplacée, aucune règle de choix de voix touchée) tient toujours. **Aucun changement d'architecture** : ni nouvelle fonction exportée, ni modification des appelants, ni `main.tsx`. Ce que ce test ne prouve pas : le comportement sur un vrai APK Android (le moment exact où le plugin s'enregistre reste hors de portée d'un test node) — la reproduction est fidèle au mécanisme, pas au chronométrage de l'appareil. Dettes voisines, **non fermées** : VOIX-04, VOIX-05 (owner Manus), GARDE-02, GARDE-03 |
| **OFF-03** | **P1 fonctionnel** | **FERMÉ** | *Ouverte ET fermée à la révision 33 (lot B4), défaut relevé par l'inventaire Manus (B4) et **reproduit sur notre socle**.* **Sans réseau, une action de commande échouait en jargon technique.** Les six gestes de `marchand/MesCommandes.tsx` (annuler, confirmer, refuser, marquer livrée, accepter et refuser une contre-offre) partaient droit au serveur ; `fetch` levait `TypeError: Failed to fetch` et le `catch` faisait `speak(e.message)` : **la marchande entendait « Failed to fetch »**. Elle ne pouvait ni comprendre, ni savoir que son geste n'était pas parti, ni savoir qu'il fallait recommencer — alors elle rappuyait. **Rouge avant correction : 18 échecs** sur `components/marchand/refusHorsLigneCommande.test.mts`, qui EXTRAIT chaque gestionnaire du vrai écran, le détype et l'EXÉCUTE avec des appels d'API espionnés | **Lot B4, ce commit** — correction **sur le socle Claude**. **Deux moments, pas un** (leçon d'OFF-01) : `reseauIndisponible()` refuse AVANT l'envoi quand on sait déjà qu'il n'y a pas de réseau, et `causeEchec()` rattrape le cas traître — le téléphone se croit connecté et l'envoi tombe quand même. Classement **par statut HTTP, jamais par le texte**, même doctrine que `doitEnfiler` : un 4xx est une vraie réponse du serveur, **son message métier reste dit tel quel** ; un 5xx ou une erreur sans statut est du transport. Module pur `services/actionReseauRequis.ts` (hors périmètre d'argent, vérifié), deux clés de catalogue `critiqueArgent` (`MARCHAND_HORS_LIGNE_ACTION`, `MARCHAND_ENVOI_TOMBE_ACTION`) — aucune phrase en dur | **AUCUNE COMMANDE NE PEUT ÊTRE PERDUE NI COMPTÉE DEUX FOIS, et c'est prouvé par exécution** : hors ligne, l'API n'est pas appelée du tout et aucun état local ne bouge ; un envoi tombé ne modifie rien non plus. **Rien n'est mis en file, et c'est un choix écrit** : une action de commande se négocie à deux, son sens dépend de l'état du serveur au moment où elle part — la rejouer plus tard sans que la marchande le sache serait pire que de lui dire non maintenant. Non-régression vérifiée dans le même test : `CommandeContext` ne mute l'état local qu'APRÈS la réponse du serveur, aucune écriture optimiste. Dettes voisines, **non fermées** : OFF-02, VOIX-04, VOIX-05, ARG-11, TEST-05, GARDE-03 |
| **VOIX-08** | **P1 fonctionnel voix** | **FERMÉ** | *Ouverte ET fermée à la révision 34 (lot B5), reprise du réglage Manus (B5) **réécrite sur notre socle**.* **Un réglage « j'entends moins » ne peut plus taire une phrase d'argent, et il ne devine plus l'importance dans le texte.** Le mécanisme Manus classait par **expression régulière française** sur le texte dit (`importancePourTexte` : « franc », « montant », « vente »…) et proposait un niveau 0 qui coupait TOUT, relecture de monnaie comprise, avec « Essentiel » **par défaut après connexion**. Mesuré sur notre catalogue : **42 des 120 messages `critiqueArgent` ne contiennent aucun des mots de cette règle** — ils auraient donc été classés « accompagnement », donc tus, par défaut ; et 140 clés sur 481 reçoivent du texte une importance différente de celle que le catalogue déclare. En langue locale, la règle ne reconnaît plus rien du tout : « il manque 500 francs » en dioula ne contient aucun de ces mots | **Lot B5, ce commit** — **rien du mécanisme Manus n'est repris**, il est lu comme référence. `i18n/voice/niveauVoix.ts` : l'importance est **LUE AU CATALOGUE**, dérivée de `critiqueArgent` — une propriété qui existe déjà, déclarée clé par clé et gardée par `test:i18n-argent` ; **aucune importance n'a été inventée pour les 475 clés**. Trois règles, prouvées sur le catalogue ENTIER : l'argent ne se tait à aucun niveau ; une clé absente ou non déclarée ne se tait jamais non plus (le doute fait parler) ; **il n'existe pas de niveau qui coupe tout**, et un ancien « 0 » resté en mémoire ne peut pas revenir par le stockage. Défaut = **« complet »**. La décision est prise dans `creerSpeakMessage`, **le dernier endroit où l'on connaît encore la clé**. La confirmation parlée du réglage lui-même vient du catalogue (`REGLAGE_VOIX_ESSENTIEL` / `REGLAGE_VOIX_COMPLET`, lues par `t()`) : **aucune phrase n'est écrite deux fois** — une clé de catalogue doublée d'une chaîne en dur à l'écran, c'est le même défaut « deux sens pour une même donnée » que ce lot combat, et il diverge à la première retouche de formulation. Tout silence part au journal (`TTS_IGNOREE`, raison `niveau-voix`) — limite **L4 de GARDE-02** traitée pour ce chemin. **Rouge avant correction : 2 échecs** sur l'entonnoir réel | **À DÉFINIR, et non inventé** : le catalogue ne distingue pas une ALERTE NON FINANCIÈRE (« Je n'ai pas bien entendu », « Ma voix ne marche pas ici ») d'un accompagnement pur (« Touche-moi et dis ce que tu as vendu »). Tant qu'aucune propriété déclarée ne le dit, « Moins bavard » tait les deux — assumé parce que ce niveau n'est **jamais imposé**, et que chaque silence est relisible dans le « Rapport de test ». La propriété à ajouter appartient à Patrick et Manus. **Observation remontée, hors périmètre B5** : le **mute global** préexistant (`voiceMuted`, « couper la voix ») coupe, lui, TOUT — y compris une relecture de montant. Il n'est pas touché ici ; c'est un geste explicite et visible, pas un niveau, mais la question mérite l'arbitrage de Patrick. Dettes voisines, **non fermées** : GARDE-02 (autres limites), VOIX-04, VOIX-05, OFF-02, GARDE-03 |
| **OFF-04** | **P2 fonctionnel** | **FERMÉ** | *Ouverte ET fermée à la révision 35 (lot B6), reprise **partielle et mesurée** de l'inventaire Manus (B6).* **Un premier lancement hors ligne s'affichait dans la police de repli du téléphone, et les images d'interface attendaient un réseau mort.** Les polices émises dans `assets/` n'entraient pas au pré-cache (le filtre ne prenait que `.js` et `.css`), alors que toute la lisibilité — mode soleil, tailles de texte, cibles tactiles — est réglée sur Inter : sans elles, la page change de métrique. `/images/*` était servi « réseau d'abord » : sur un réseau mort mais **pas coupé**, chaque affichage attendait le délai d'expiration avant de retomber sur le cache. **Rouge avant correction** : `check:precache-budget` sortait « aucune police au pré-cache » | **Lot B6, ce commit** — **le « tout précacher » de Manus est refusé**, et le refus est chiffré, pas argumenté. MESURÉ SUR LE BUILD : pré-cache **10,19 Mo → 10,45 Mo**, soit **+271 Ko** (10 polices, +2,6 %) ; les **4 gros chunks restent dehors (1,84 Mo)** ; les **137 clips de voix (5,62 Mo) sont inchangés** et continuent de se remplir en tâche de fond, sans retarder la première ouverture. `/images/*` passe **cache d'abord sans être pré-caché** : coût d'installation **nul**, et plus aucune attente sur un réseau mort. Deux gates nouveaux : `check:precache-budget` (après build — ventile par type, affiche ce qui reste volontairement dehors, et **refuse** au-delà de 12 Mo au total / 6 Mo bloquant, ou si la voix tombe sous 100 clips) et `test:precache-doctrine` (dans `verify`, sans build — la règle du plafond, l'absence d'ajout non filtré, et la voix qui ne se perd pas) | **Ce qui n'est PAS prouvé ici, et qui se mesure au terrain** : le temps réel d'installation sur un réseau lent. Ce gate borne la TAILLE, qui le détermine, pas le débit. **Constat, non traité** : le code référence une quinzaine d'images (`/images/tata-nanti-lou.png`, `/images/tantie-*.png`…) **absentes du build** — seules 3 icônes PWA existent, dont 2 identiques à l'octet. Hors périmètre B6 (ce n'est pas du pré-cache), signalé pour arbitrage. Dettes voisines, **non fermées** : OFF-02, VOIX-04, VOIX-05, UI-05, ARG-11, TEST-05, GARDE-03 |
...
| **GARDE-03** | **P2 outillage** | **OUVERT** | *Ouverte à la révision 30, **constat du contre-audit du lot B2**, pas de l'auteur du lot.* **Le garde-argent peut être satisfait à la main : il croit son fichier de preuve sur parole.** `ci/garde-argent.mjs`, autour de la l. 762, lit le fichier passé à `--preuve` et vérifie **seulement qu'il contient les chaînes de commande** exigées (`fournis = new Set(readFileSync(PREUVE).split('\n')…)`, puis `manquants = exigees.filter(…)`). Il ne vérifie jamais que ces commandes ont TOURNÉ, ni qu'elles sont sorties à zéro, ni quand. Le fichier n'est même pas commité : il n'en reste aucune trace après coup. Un `printf` de vingt-sept lignes suffit donc à faire dire au gate « les 27 invariants exigés ont tourné » | **Pourquoi c'est grave ici plus qu'ailleurs.** La doctrine du projet est « sur l'argent, la preuve doit TRAVERSER ». Ce gate est précisément celui qui l'impose aux autres : qu'il repose, lui, sur la bonne foi de qui l'exécute en fait un gate **déclaratif**, et une exigence déclarative se dégrade en formalité au bout de quelques lots. Le mécanisme d'anti-assouplissement (empreinte des gardes) et le recalcul du périmètre, eux, ne se contournent pas : c'est l'asymétrie qui doit être corrigée | **Condition de clôture — à décider, PAS implémentée dans B2** (modifier le garde dans le même diff qu'un changement du chemin d'argent est exactement ce qu'il refuse, et c'est un chantier à soi). Pistes à arbitrer : que le garde LANCE lui-même les invariants exigés au lieu de lire une liste ; ou que la preuve soit un artefact vérifiable — code de sortie, horodatage et empreinte de l'arbre de travail par commande — produit par le garde et non par l'agent ; ou que la CI soit la seule source admise. Dettes voisines : GARDE-01, GARDE-02, TEST-05 |
