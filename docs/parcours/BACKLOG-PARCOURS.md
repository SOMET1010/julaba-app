...
| AKW-02 | **DIAGNOSTIC CORRIGÉ À LA REMESURE DU 23/09** (Patrick l'a confirmé) : les écrans d'entrée n'étaient plus muets depuis AKW-01. **Le défaut réel était que chacun s'était fait SA PROPRE porte** vers `audioManager`, sans qu'aucune règle ne dise qui avait le droit de l'emprunter — un écran d'institution pouvait faire pareil. Trois contournements sans règle, c'est pire qu'un refus : le refus, lui, se voyait. Une seule primitive `parlerAvantConnexion(ecran, texte, emettre?)` remplace les portes, et le garde interdit d'en rouvrir une. | **FERMÉ** |
| AKW-05 | **QUATRIÈME PORTE, trouvée en fermant les trois autres** : `LoginPassword` parlait AUSSI par `audioManager.speak` en direct (l. 54), à côté de son propre `parle()`. **Deux portes dans un seul fichier** — la preuve qu'une porte non nommée s'en fait toujours une autre. | **FERMÉ** |
...
| NUM-03 | **ÉNONCÉ CORRIGÉ À LA REMESURE DU 23/09.** Le backlog disait « le pavé numérique est silencieux ». C'est inexact pour l'étape NUMÉRO : elle vibre à chaque chiffre, et son silence VOCAL est **voulu** (NUM-02 — on ne dit jamais un chiffre du numéro à voix haute au marché). **Le vrai trou était le CODE** : la branche PIN de `handleKeyPress` posait le chiffre et s'arrêtait là — ni vibration, ni son. Quatre chiffres à chaque connexion, le geste le plus répété de l'application, sans aucun signe que l'appui a compté. Asymétrie nommée nulle part. | **FERMÉ** |
| NUM-05 | **La garde VOICE-01 a refusé le déplacement d'un appel de parole**, et elle avait raison : `parle('Effacé.')` devenu `parle(r.ditVoixHaute)` casse la trace figée de cet écran. Ce qui est dit à une marchande ne doit pas changer de forme au détour d'un refactor. Le lot a donc été **restreint au retour tactile** ; la parole est restée exactement où elle était. Le champ `ditVoixHaute` de la règle n'est pas consommé par cet écran — il documente ce qui a le droit d'être dit, pas ce qui l'est. | **HORS PÉRIMÈTRE JUSTIFIÉ** — desserrer VOICE-01 est une décision de Patrick |
...
| VOX-03a | **ON DEMANDE SON PRIX À UNE NON-LECTRICE AVEC UN CLAVIER.** Constat terrain de Patrick, 24/09 : « 2 tas de gombos » → l'application demande le prix, « mais avec une interface pour saisir — si je ne sais pas lire ? ». `SaisieGuidee.tsx` contient **ZÉRO** occurrence de `Mic` : un pavé de chiffres, un champ texte, et **rien pour parler**. C'est l'écran du PRIX, donc de l'argent, et c'est un mur pour la personne même pour qui l'application est faite. **Fermé le 24/09** : `services/montantDit` lit le montant dans ce qu'elle dit — et sans nombre clair rend `null`, le champ reste vide et elle redit (on ne devine JAMAIS un prix à sa place, STK-02). `BoutonDirePrix` réutilise `startLiveDictation` (VOICE-01 : importé, jamais modifié), coupe la parole avant d'ouvrir l'oreille (verrou parole/écoute), plafonne à 12 s, et si le micro ne veut pas : « Le micro ne répond pas. Tape le prix. » — rien n'est bloqué. Contre-essais du câblage : bouton débranché, ouverture automatique retirée, micro décoratif remis, montant relu à la main, dictée réécrite — **chacun rouge à l'endroit attendu**. | **FERMÉ** |
| VOX-03b | **ET J'AI AJOUTÉ UN MICRO DÉCORATIF.** `AjoutProduitGuide.tsx:111` (écrit par moi le 23/09) affiche `<Mic aria-hidden> ou dis-le à Tantie` **dans un `<p>`, non cliquable**. Une image qui promet la voix sans la donner — exactement la faute que ce dépôt combat (CAI-08 : un bouton câblé sur le vide ; ACC-05 : un clip absent qui passe pour joué). Pire qu'un écran muet : il ment. **Arbitrage de Patrick, 24/09 : le micro s'ouvre TOUT SEUL, Tantie pose la question à voix haute, elle répond en parlant ; le clavier reste, en second — le geste par défaut doit être celui qu'elle sait faire.** Fermé avec VOX-03a par le même bouton réel (`BoutonDirePrix`), branché dans les DEUX écrans du prix ; l'icône décorative est supprimée, et une garde interdit qu'elle revienne. | **FERMÉ** |
| VOX-03c | **UN NOM DE PRODUIT SEUL N'EST COMPRIS PAR RIEN.** Mesuré : « gombo », « gombos », « du gombo », « gombo 1000 » → tous `null` → « Je n'ai pas compris ». Il faut une quantité (« 2 gombos ») ou un verbe (« je vends du gombo »). Or nommer ce qu'on vend est le geste le plus naturel au marché. | **OUVERT** — arbitrage : demander la suite / vendre 1 / ouvrir la saisie |
| ARG-12a | **« À » NE TRANCHE RIEN, ET LE PRENDRE POUR UNE DÉCISION ÉTAIT DÉJÀ DEVINER.** `extraction.ts` lisait « 2 tas de piments à 1000 » comme 1 000 **le tas**, en le documentant (« c'est son mot qui tranche, pas notre devinette »). Au marché, ce même « à 1000 » veut dire 1 000 le tas OU 1 000 pour les deux, selon les habitudes. **Arbitrage de Patrick du 23/09** : quantité > 1 et aucun mot qui tranche → **on demande**. | **FERMÉ** |
| ARG-12b | **ET ON NE DEMANDE QUE LÀ.** Patrick : « l'ambiguïté n'est levée que lorsqu'elle existe vraiment » — une question de trop sur chaque vente ferait abandonner l'outil aussi sûrement qu'un faux prix. Marqueurs explicites reconnus : *chacun, chaque, le tas/kilo/sac/panier…, l'unité, la pièce* → unitaire ; *le tout, en tout, les deux/trois…* → lot. Quantité ≤ 1 → aucune question possible. | **FERMÉ** |
| ARG-12c | **#6 tombe avec #3, et c'est MESURÉ, pas supposé.** « 5 tas de gombos à 1500 » posait **7 500 F** au panier sans confirmation — 6 000 F d'écart si elle voulait dire 1 500 pour les cinq. Le panier reste désormais **vide** et la question part : « 1 500 francs, c'est le prix d'un seul, ou de tous les 5 ? ». Le mécanisme de levée existait **et fonctionnait** ; il ne recevait jamais la main. | **FERMÉ** |
| ARG-12d | **DEUX PIÈGES DE MISE EN ŒUVRE, trouvés en mesurant.** (1) `\b` ne s'accroche pas après un caractère accentué en JS : `unité\b` n'a jamais matché. (2) La décision se prenait **avant** l'heuristique de dernier recours, qui assigne encore la quantité : « 2 tas à 1000 » était donc lu comme une seule unité. Les deux n'auraient jamais été vus sans exécuter les vraies fonctions. | **FERMÉ** |
| VOX-02a | **L'ÉCRAN DEVENAIT MORT APRÈS LA PREMIÈRE VENTE.** `VoiceState` a **sept** valeurs, `handleMicClick` en traitait **six** : `confirming` n'apparaissait dans **aucune** branche, alors qu'il est posé 4 fois — toutes sur le chemin d'une confirmation FINANCIÈRE ou d'une question. **Et vendre EST une intention financière** : le piège se refermait sur le geste le plus courant. L'appui ne faisait rien : pas d'erreur, pas de son, pas de retour. Trouvé sur le téléphone de Patrick, 23/09 20h57. | **FERMÉ** |
| VOX-02b | **ET IL N'Y AVAIT PAS DE SORTIE.** Le bouton « Parler encore à Tantie » ne s'affichait que si `isDone \|\| isError` — **deux états sur sept**. Micro inerte + aucune sortie = écran mort. Patrick : « Je dois sortir mais il n'y a pas de vrai bouton pour sortir. » Pour une marchande qui ne lit pas, il n'existe pas de « sortir et revenir » : elle repose le téléphone. | **FERMÉ** |
| VOX-02c | **LA FORME ÉTAIT LA CAUSE, PAS L'INATTENTION.** Une suite de `if/else if` sur un type à sept valeurs ne se plaint jamais quand il en manque une. La règle est désormais un `switch` exhaustif avec `assertJamais` : **un état oublié NE COMPILE PAS**. Contre-essai : retirer `case 'confirming'` → le compilateur refuse. | **FERMÉ** |
| VOX-02d | **Une assertion figée doit être remplacée par une plus forte.** `caisseMicroPermanent.test.mts` exigeait `onClick={handleMicClick}` **en direct** — c'était exiger le trou lui-même, puisque `handleMicClick` ne traite pas `confirming`. Remplacée par deux assertions (le bouton passe par la règle des sept états, ET cette règle mène bien au moteur). `--figer-gardes` est **la décision de Patrick, jamais d'un agent** : Patrick l'a lancé lui-même le 24/09 (`9131eb5`) : 148 gardes figées, **aucune assertion retirée, renommée ni désarmée**. | **FERMÉ** — signé par Patrick |
...
```
rouge avant : ✗ bouton rebranché sur l'ancienne saisie de vente
              ✗ prix pré-rempli à 400 en repli  (4 assertions)
              ✗ second chemin d'écriture (localStorage)
              ✗ LE PÉRIMÈTRE A BOUGÉ sans déclaration (2 entrés)
après gel   : noyau 89 → 91, symboles 37 (inchangés)
              ENTRÉS : AjoutProduitGuide.tsx, premierProduit.ts — zone caisse-context
              SORTIS : aucun          RECLASSÉS : aucun
              zones, symboles, invariants, chaineTestCiGelee, racinesScannees : inchangés
              2044 assertions figées — aucune retirée, renommée ni désarmée
              chaîne test:ci : identique à f0c965c, 44 maillons
```

`useCaisse().addProduct` est **validé par Patrick** comme le bon chemin pour ce
parcours : seule cette primitive rafraîchit `products`, donc son étal. Ne pas la
remplacer par celle du stock.
...
| BO-02 | **Les autres sources du contexte avalaient leur erreur.** Mesure du 24/09 : **6** avalements réels — et non 7 : le septième `console.error('[BO]', e)` compté était dans un commentaire. Sur `missions`, `audit`, `utilisateurs BO`, `institutions`, `signalements`, `notifications`, aucune n'avait **le moindre** état de lecture. Elles portent maintenant celui de BO-01 — mêmes trois états, aucun second mécanisme. Il ne reste **0** `console.error('[BO]')` dans le code. | **FERMÉ** |
| BO-03 | **Une panne totale du back-office se présentait à l'agent comme une bonne nouvelle.** Trois sources injoignables → trois compteurs à 0 → liste d'alertes vide → l'écran affichait « Aucune alerte activée — Tout est en ordre. Aucune action urgente requise. » Et les quatre barres d'objectifs affichaient **0 %** face à des cibles de 75 à 95 % : une barre à zéro devant une cible se lit comme un échec de terrain, pas comme une absence de mesure — or c'est exactement le nombre sur lequel une institution taille un programme. Cinq sections passent désormais par `sectionDe` : **le calcul n'est même pas appelé** tant que tout n'est pas lu, et la phrase rassurante n'est dite que si les trois sources ont répondu. | **FERMÉ** |
| BO-06 | `refreshRoleCounts` n'a **aucun** `try/catch` : un rejet laisse sept zéros fabriqués dans les filtres de l'écran Acteurs, plus un rejet de promesse non traité. Dette VOISINE nommée en fermant BO-02. | **OUVERT** |
| BO-07 | Le ticker du tableau de bord fabrique « Aucune transaction — 0 » sur une lecture ratée. Hors du périmètre de BO-03 : ni alerte, ni graphique, ni barre. Dette VOISINE. | **OUVERT** |
| BO-08 | `refreshSignalements` et `refreshNotifications` sont du **code mort** : jamais appelées, listes non exposées. Leur état de lecture existe mais restera `attente` à vie — **4 sources vivantes ont été réparées, pas 6**. À supprimer ou à brancher : décision produit. | **OUVERT** |
| BO-09 | Les 36 écrans hors tableau de bord ne lisent pas encore `lectures` : BO-02 arrête l'avalement à la source, mais Missions, Audit, Utilisateurs BO et Institutions rendent toujours `[]` comme « rien ». L'information existe, personne ne l'affiche. C'est la vraie surface restante. | **OUVERT** |
...
| STK-20 | **LE CHAMP DU NOM NE RETENAIT QU'UN SEUL CARACTÈRE.** Recette DTDI du 24/09, reproduit et prouvé : elle tape « T », l'écran bascule sur l'unité, **l'input est démonté sous ses doigts**, et son produit s'appelle « T ». Le champ d'unité libre avait le même défaut — « bassine » devenait « b ». **Faute que j'ai introduite avec STK-03e**, et elle rendait toute la ligne « Ajouter Tomate, Banane et Riz sac » impossible : rien de ce qui vient après n'était testable. **La cause de fond** : une même donnée portait deux sens. « Que manque-t-il au produit ? » et « quel écran afficher pendant qu'elle tape ? » ne sont pas la même question, et `etapeCourante` répondait aux deux. Elle ne change pas — elle reste la bonne réponse à la première, et STK-05 s'en sert. Ce qui est neuf : **avancer est un GESTE** (`peutValider`, `etapeSuivante`), et l'écran retient où elle en est. **Arbitrage de Patrick, option C** : le grand bouton vert « C'est bon » pour elle, la touche OK du clavier pour qui va vite. Le contre-essai qui remet le défaut d'origine est d'abord resté **VERT** — le mot `etapeVue` survivait à sa transformation en calcul ; l'assertion a été dédoublée et exige désormais un état ET la disparition de `etapeCourante(brouillon)` de l'écran. | **FERMÉ** |
| STK-05 | **LE GROS BOUTON « AJOUTER EN PARLANT » DICTAIT UNE PHRASE QUE RIEN NE COMPRENAIT.** L'écran imprimait « dis : « ajoute 10 piments à 500 » » et `intentLocal` rendait `null` sur cette phrase exacte ; `ajouter_stock` n'avait **aucun producteur** dans le dépôt. Un bouton qui lui donne tort. **La cause n'était pas une grammaire manquante** : `extraire()` comprenait déjà tout (produit, quantité, montant, unité) — seul `intention` restait vide, et `intentLocal`, qui ne fabrique que `vendre` et `depense`, jetait le reste. L'information existait, l'aval la jetait. **Arbitrage de Patrick du 24/09, option A** : ce qui est compris préremplit `AjoutProduitGuide`, on ne demande que ce qui manque, rien n'entre dans l'étal avant confirmation complète, et aucune unité, prix ou produit n'est inventé. Le bloc mort a été retiré — il portait en plus une **seconde naissance** (catégorie, unité et seuil repris d'un catalogue générique, stock non compté) : c'était STK-03g, jamais corrigée parce qu'inatteignable. **Le câblage ne pouvait PAS passer par `onAction`** : mesuré, quand `intentLocal` rend `null`, `handleResponse` n'est jamais appelé — la garde regex serait passée au vert sans que rien ne marche. On reprend le chemin validé par VOX-03 : la dictée directe, VOICE-01 intact. | **FERMÉ** — sous réserve de la signature `--figer-gardes` de Patrick |
| STK-06…STK-19 | Treize autres dettes du stock, **mesurées le 24/09 mais non revérifiées une à une** — preuves statiques (fichier + ligne) dans `docs/parcours/MESURE-STOCK-24-09.md`. Les plus chères : l'écran affirme « Aucun produit » et cinq zéros sans avoir rien pu lire (STK-14 — c'est CAI-01/CAI-06, fermés sur la caisse et **vivants sur le stock**) ; deux routes écrivent la même ligne produit avec deux règles opposées sur « la case vidée » (STK-06) ; le produit qu'elle vient de poser lui revient **« ✕ Rupture », en rouge, avec vibration** (STK-09). **Prouver avant de corriger.** | **OUVERT** |
