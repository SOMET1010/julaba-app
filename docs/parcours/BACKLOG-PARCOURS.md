...
| STK-05 | **LE GROS BOUTON « AJOUTER EN PARLANT » DICTE UNE PHRASE QUE RIEN NE COMPREND.** L'écran du stock imprime lui-même « dis : « ajoute 10 piments à 500 » » — et `intentLocal` rend `null` sur cette phrase exacte. `ajouter_stock` a **4 occurrences dans le dépôt, toutes consommatrices, AUCUN producteur** ; le mot « ajoute » est même dans `MOTS_PAS_UNE_VENTE` : il FERME une porte au lieu d'en ouvrir une. Elle dit ce qu'on lui dicte et reçoit « Je n'ai pas bien compris » — ce n'est pas un bouton muet, c'est un bouton qui lui donne tort. Et tout le correctif STK-02c vit dans ce bloc inatteignable. **Vérifié de ma main le 24/09** ; c'est le **#7 du retour terrain**. | **OUVERT** |
